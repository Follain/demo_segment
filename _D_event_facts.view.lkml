view: event_facts {
  derived_table: {
    ...
    sql: select t.received_at
        ...
        , first_value(t.referrer ) over (partition by s.session_id order by case when t.referrer  is not null then 0 else 1 end,t.received_at rows between unbounded preceding and unbounded following) as first_referrer
